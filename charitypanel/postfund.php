...
<?php
$nameErr = $descriptionErr = $goalErr = $date1Err = $nameErr = $typeErr = $date2Err = $cityErr = $stateErr = $zipErr = $bedsErr = $bathsErr = $priceErr = $rmNumErr = "";
...
if(isset($_POST['action']))
{
  ...
  if($_POST['action']=="list")
  {
    ...
    $curr = 0;
    $sql = "INSERT into `fundraiser` (`cid`, `name`, `description`, `date2`, `current`, `goal`) values('".$cid."', '".$name."','".$description."','".$date2."','".$curr."','".$goal."')";
...
<body>
  ...
  <div id = "mainBox">
      ...
      <form name = "master" action="" method = "post">
        <h2 id = "headTitle">Create a Fundraiser</h2>
        <div id = "inputSec" class = "textInput">
          <?php echo $nameErr ?>
          <input class = "auto" type="text" name="name" pattern = ".{1,50}" placeholder="Fundraiser Name (1-50 characters)" required>
          <?php echo $descriptionErr?>
          <textarea readonly = "readonly" cols = "4" rows = "1" id = "counter"></textarea>
          <br>
          <input  onkeyup="textCounter(this,'counter',255);"  pattern = ".{1,255}"  class = "auto" type="text" name="description"  placeholder="Fundraiser Description (1-255 characters)" required>
          <?php echo $date2Err ?>
          <input name="date2" placeholder="End Date"  class="datepicker-here" data-position="right top"  data-language='en'   required>
          <?php echo $goalErr?>
          <input class = "auto" type="text" name="goal"  pattern = "[0-9]{1,7}"  placeholder="Funding Goal in $ (numbers only)">
        </div>
